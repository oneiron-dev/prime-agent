...
- `mechanism`: Internal audit value: `local`, `remote`, or `extension`
- `remoteCompaction`: Versioned, provider/API/model-scoped opaque Responses items used only for native replay on the matching branch and model
- `fallback`: Fixed classification when a declared remote endpoint is concretely unsupported or unavailable and local compaction ran instead

Opaque remote state remains in the authoritative session JSONL for resume and branch replay. UI/daemon snapshots, HTML, trace uploads, agent observation, extension events, and public compaction results omit it. Portable JSONL exports omit remote checkpoint entries so reopening the export reconstructs the original normalized history.
