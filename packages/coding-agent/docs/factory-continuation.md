...
`OneironSuccessor` contains exact `requestId`, observed `planRevision`, bounded `reason`, one to eight evidence pins, and one of these `next` instructions:
...
- `resume-judgment`: one to three pinned substantive supplemental evidence records, and optional `{requestId,reconciliation}` for the core `ManagementReconciliation` path. This never deletes claims. The next pass regenerates the verified stage binding with the new evidence before bounded management.
