...
`--pause-file /absolute/owner-pause` adds an independent external fence: its existence blocks new dispatch and operator mutation commands. Tick reconciliation of active attempts may still record running, terminal or uncertain state. `resume` neither removes nor overrides the pause file. Archive that sentinel only as part of the owner's explicit resume procedure. Do not remove an existing wave pause to run an unrelated fixture.
