...
const WORK: Record<OneironManifest["stage"]["kind"], string> = {
	...
	"publish-update":
		"Request actual Qodo and Codex review of the newly published exact head using the authorized review-request helper, once. After those requests, request CodeRabbit on the same head with one PR comment `@coderabbitai review`, never with credits. Record and ignore a failed or refused CodeRabbit request; do not retry it, block, or open a wake for it. Retain its request receipt and every prior unresolved finding. Prepare collect for that exact head. Do not accept from request/check status.",
	collect:
		"Inspect completed exact-head Qodo/Codex coverage and all substantive CodeRabbit, Cursor and Greptile findings. Unavailable advisory output is ignored and never causes a wait, wake or retry. Prepare bounded triage with prior obligations, or another bounded collection when review is pending. Missing/skipped/quota review is not completed. Preserve the request deadline and consume-once reminder through the authorized coordinator helper.",
	"review-acceptance":
