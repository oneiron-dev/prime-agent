...
import {
	...
	formatUnsupportedRlmSubagentThinkingLevelError,
	...
	resolveRequestedRlmSubagentThinkingLevel,
...
export class AgentSession {
	...
	async handleRlmHeartbeatHostRequest(
		...
		switch (type) {
			...
			case "rlm_heartbeat.delete": {
				...
				const cancellation = await controller.deleteRlmHeartbeat(payload.id);
				if (!cancellation) {
	...
	private _onIpythonStateRestored(result: RestoreResult): void {
		...
		if (result.restored.length > 0) {
			lines.push(
				`Your Python kernel state was revived from your previous session. These names are available again: ${summarizeKernelNames(result.restored)}.`,
			);
		} else {
			lines.push(
	...
	private async _performCompaction(options: {
		...
		const summaryCall = async <T>(
			...
			if (requestId === undefined) return call(requestHeaders);
			try {
				const result = await call({ ...requestHeaders, ...modelRequestHeaders(requestId) });
		...
		const compactLocally = async (localPreparation: CompactionPreparation): Promise<CompactionResult> => {
			const summarization = (await this._resolveAuxiliaryModel(
				"compaction summary",
				{ model, apiKey, headers },
				estimateSummaryRequestTokens(localPreparation, customInstructions),
			)) ?? { model, apiKey, headers };
			return compact(
				localPreparation,
				summarization.model,
				...
				signal,
				undefined,
				(call) => summaryCall(call, summarization.headers ?? {}),
		...
		try {
			...
			if (extensionCompaction) {
				...
				try {
					...
					compactionResult = await compactLocally(localPreparation);
	...
	isRetainedRlmChildSession(childId: string, session: AgentSession): boolean {
		return this._rlmChildSessions.get(childId)?.session === session;
	}
	...
	private _observeRlmRunDeletionCleanup(
		...
		const observer = cleanup.then(
			async () => {
				// Cache pruning is best effort and runs only after the child was really
				// disposed. It must never turn an accepted deletion into a failure.
				const prune = await this._pruneDeletedRlmChildKernelCaches(subagent, session);
				if (prune.outcome === "failed") {
	...
	/**
	 * Attach a cache-cleanup result only when it carries information: a no-op
	 * (nothing to prune, or no session file to prune from) keeps the plain
	 * deletion result shape.
	 */
	private _rlmDeleteResultWithCleanup(
		subagent: RlmSubagentRegistryEntry,
		cleanup: ChildKernelCacheCleanupResult,
	): RlmDeleteSubagentResult {
		if (cleanup.outcome === "not_found" || (cleanup.outcome === "skipped_invalid" && cleanup.files === 0)) {
			return { subagent };
		}
		return { subagent, kernel_cache_cleanup: cleanup };
	}
	...
	private async _pruneDeletedRlmChildKernelCaches(
		...
		try {
			return await pruneDeletedChildKernelCaches(sessionPath);
		} catch (error) {
	...
	private async _deleteResolvedRlmSubagent(subagent: RlmSubagentRegistryEntry): Promise<RlmDeleteSubagentResult> {
		...
		if (run) {
			...
			if (run.status === "error" && !liveSession && run.settled) {
				...
				return this._rlmDeleteResultWithCleanup(subagent, cleanup);
			}
		...
		const cleanup = await this._pruneDeletedRlmChildKernelCaches(subagent, retained?.session);
		return this._rlmDeleteResultWithCleanup(subagent, cleanup);
	}
	...
	async waitForRlmQuiescence(externalSignal?: AbortSignal): Promise<void> {
		...
		try {
			...
			// Drop the local rejector before aborting: on a successful return nothing
			// races `cancelled` anymore, so rejecting it here would surface as an
			// unhandled rejection. Child propagation listeners stay installed, so the
			// abort below still cancels sibling recursive waits owned by this barrier.
	...
	private async _startRlmChildRun(
		...
		const requestedThinkingLevel = resolveRequestedRlmSubagentThinkingLevel(rawReasoning, rawThinking);
		...
		try {
			...
			if (requestedThinkingLevel !== undefined) {
				...
				if (!supported.includes(requestedThinkingLevel.level)) {
					throw new Error(
						formatUnsupportedRlmSubagentThinkingLevelError(
