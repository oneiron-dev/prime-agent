	...
	private readPrivatePrimeAuthorizationCache(): PrivatePrimeAuthorizationCache | undefined {
		...
		try {
			...
			if (parsed.models !== undefined) {
				...
				const models =
					parsed.models.length === 0 ? [] : parseModelCatalog({ schemaVersion: 1, models: parsed.models }).models;
				...
				return { fingerprint: parsed.fingerprint, models: privateModels, refreshedAt: parsed.refreshedAt };
			}
		...
		try {
			writeFileAtomicSync(
				...
				JSON.stringify({
					...
					data,
					// Preserve live reasoning controls instead of rebuilding them from stale templates.
					models: createModelCatalog(cache.models).models,
