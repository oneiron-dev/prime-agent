...
export type CompactionMechanism = "local" | "remote" | "extension";

export interface RemoteCompactionState {
	version: 1;
	provider: string;
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	tokensBefore: number;
	/** Mechanism used for this checkpoint. Absent on pre-feature entries, which are local summaries. */
	mechanism?: CompactionMechanism;
	/** Versioned opaque Responses items, scoped to the provider/API/model that created them. */
	remoteCompaction?: RemoteCompactionState;
	/** Concrete remote incompatibility that caused a local fallback. */
	fallback?: CompactionFallback;
...
export function isValidRemoteCompactionState(value: unknown): value is RemoteCompactionState {
...
function createRemoteCompactionMessage(
	state: RemoteCompactionState,
	timestamp: string,
): OpenAIResponsesCompactionMessage {
	return {
...
/** Remove provider-opaque state from UI/export/telemetry projections. */
export function projectAgentMessagesForExternalUse(messages: readonly AgentMessage[]): AgentMessage[] {
	return messages.filter((message) => message.role !== "openaiResponsesCompaction");
}

/** Remove provider-opaque state while retaining compaction audit metadata. */
export function projectSessionEntryForExternalUse(entry: SessionEntry): SessionEntry {
	if (entry.type === "compaction") {
		const { mechanism: _mechanism, remoteCompaction: _remoteCompaction, fallback: _fallback, ...publicEntry } = entry;
		return publicEntry;
	}
	if (entry.type === "model_change" && entry.api) {
		const { api: _api, ...publicEntry } = entry;
		return publicEntry;
	}
	if (entry.type === "message" && entry.message.role === "openaiResponsesCompaction") {
		return {
			...entry,
			message: { ...entry.message, items: [] },
		};
	}
	return entry;
}

export function projectSessionContextForExternalUse(context: SessionContext): SessionContext {
	const { modelApi: _modelApi, ...publicContext } = context;
	return { ...publicContext, messages: projectAgentMessagesForExternalUse(context.messages) };
}

export function projectSessionTreeForExternalUse(nodes: readonly SessionTreeNode[]): SessionTreeNode[] {
	return nodes.map((node) => ({
		...node,
		entry: projectSessionEntryForExternalUse(node.entry),
		children: projectSessionTreeForExternalUse(node.children),
	}));
}

export function projectSessionFlatTreeForExternalUse(nodes: readonly SessionTreeFlatNode[]): SessionTreeFlatNode[] {
	return nodes.map((node) => ({ ...node, entry: projectSessionEntryForExternalUse(node.entry) }));
}
...
export function buildSessionContext(
	...
	// Extract settings and select the newest checkpoint compatible with the active model.
	let thinkingLevel = "off";
	let serviceTier: ServiceTier = "default";
	let model: SessionContext["model"] = null;
	let modelApi: Api | undefined;
	...
	const compaction =
		compactions
			...
			.reverse()
			.find((entry) => {
				if (entry.mechanism !== "remote") return true;
				return entry.remoteCompaction
					? remoteCompactionMatchesModel(entry.remoteCompaction, model, modelApi)
					: false;
			}) ?? null;
...
const SESSION_SCAN_RESUME_TAIL_BYTES = 4096;
...
function scannedPrefixIntact(filePath: string, state: SessionScanState): boolean {
	...
	try {
		...
		return (
			readBytesSync(filePath, 0, state.head.length).equals(state.head) &&
			readBytesSync(filePath, start, state.offset).equals(state.tail)
		);
	} catch {
		return false;
	}
...
async function scanSessionLines(filePath: string, state: SessionScanState, size: number): Promise<Buffer | undefined> {
	...
	for await (const lineBuffer of readLinesAsBuffers(filePath, { start: state.offset, end: size - 1 })) {
		...
		if (state.head.length < SESSION_SCAN_RESUME_TAIL_BYTES) {
			state.head = Buffer.concat([
				state.head,
				lineBuffer.subarray(0, SESSION_SCAN_RESUME_TAIL_BYTES - state.head.length),
				NEWLINE_BUFFER,
			]).subarray(0, SESSION_SCAN_RESUME_TAIL_BYTES);
		}
...
export class SessionManager {
	...
	appendModelChange(provider: string, modelId: string, api?: Api): string {
