...
export class KernelManager {
	...
	/** Linux starttime captured at fork admission; prevents PID-reuse group signals. */
	private forkedKernelStartTime?: string;
	...
	private async doStart(startOptions: KernelStartOptions): Promise<void> {
		...
		if (isForkServerEnabled()) {
			try {
				...
				this.forkedKernelStartTime = undefined;
	...
	private async forkedKernelDead(probed: ForkedKernelHandle | undefined, timeoutMs?: number): Promise<boolean> {
		...
		try {
			...
			// A denied signal proves the pid still exists and is not ours to observe.
		...
		if (this.kernel) {
			...
			try {
				if (directPid !== undefined) {
					// The direct spawn is detached, so its descendants share this group.
					signalKernelProcessTree(directPid, killSignal);
					signaled = true;
				} else {
			...
			const forkedStartTime = this.forkedKernelStartTime;
			...
			void forked
				...
				.then((outcome) => {
					...
					recordOrphanProcessState(forked.pid, false);
					// The forkserver signals only the child pid. Forked kernels call
					// setsid(), so their owned descendants need an explicit group signal,
					// but only while the pinned identity still matches: a recycled pid
					// must never be targeted.
					if (process.platform === "linux" && !isSameLinuxProcessIdentity(forked.pid, forkedStartTime)) {
						return;
					}
					try {
						signalKernelProcessTree(forked.pid, killSignal);
					} catch {
		...
		try {
			const r = await this.enqueueExecute(
				code,
				{
					...
					internal: true,
					// Guardrail deadline for every snapshot, including callers that pass no
					// tighter per-call bound; whichever fires first aborts the cell.
