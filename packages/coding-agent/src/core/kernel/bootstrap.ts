...
async function bootstrapVenv(
	...
	await runInstall(uv, ["venv", venv, "--python", PYTHON_VERSION], options);
	await runInstall(
		uv,
		[
			"pip",
	...
	// Record the base before skill sync so an interrupted sync does not rebuild the venv.
...
async function ensureKernelPythonUncached(
	...
	const readyForCaller = async (): Promise<boolean> =>
		pythonSkills.length === 0
			? kernelBaseReady(python, venv, runtimeIdentity, options)
			: kernelReady(python, venv, runtimeIdentity, pythonSkills, options);
