...
export async function* readLinesAsBuffers(filePath: string, start = 0, end?: number): AsyncGenerator<Buffer> {
	const pendingParts: Buffer[] = [];
	let pendingBytes = 0;
	for await (const chunk of createReadStream(filePath, { start, ...(end === undefined ? {} : { end }) })) {
