...
import { DaemonSessionRecoveringError, deserializeDaemonError } from "../daemon/daemon-errors.js";
import {
...
export class AgentsViewMode implements Component, Focusable {
	...
	handleInput(data: string): void {
		...
		if (!this.replyTarget && this.editor.getText().length === 0) {
			if (this.keybindings.matches(data, "app.agents.expand")) {
				...
				if (row && isAgentsViewSessionRow(row) && (row.kind === "subagent-summary" || row.descendantCount > 0))
		...
		if (!this.replyTarget && this.keybindings.matches(data, "app.agents.togglePin")) {
			this.togglePinSelection();
			return;
		}
		if (!this.replyTarget && this.keybindings.matches(data, "app.agents.reorderUp")) {
	...
	private reorderSelection(direction: -1 | 1): void {
		const row = this.rows[this.selectedIndex];
		if (!row?.selectable || (row.kind !== "agent" && row.kind !== "subagent")) return;
		...
		const visibleIndex = visiblePeers.findIndex((candidate) => candidate.identity === row.identity);
		if (visibleIndex < 0 || visibleIndex + direction < 0 || visibleIndex + direction >= visiblePeers.length) return;
		const fullRows = buildAgentsViewRows(
			this.scopedRecords,
		...
		const fullPeers = fullRows.filter(
			(candidate) => candidate.selectable && getAgentsViewReorderGroup(candidate) === group,
		);
		const order = this.persistentState.manualOrder ?? {};
	...
	private queryChanged(): void {
		...
		if (this.getActiveSearchQuery().trim() && !this.getSelectedSessionRow()) {
			const firstSession = this.rows.findIndex(isAgentsViewSessionRow);
			if (firstSession >= 0) this.selectedIndex = firstSession;
		}
		this.syncSelectedRowState();
		this.ui.requestRender();
	}
	...
	private async killSubagent(pending: PendingKillSubagent, currentRow: AgentsViewSessionRow): Promise<void> {
		...
		try {
			...
			const canRefresh = this.rosterStore !== undefined && !this.reconnectPromise && !this.daemonShutdownReceived;
			const refreshed = (await this.refreshSessions()) ?? canRefresh;
	...
	private renderSessionRows(width: number, maxRows: number): string[] {
		...
		if (this.rows.length === 0) {
			return [
				theme.fg("dim", this.getActiveSearchQuery().trim() ? "No sessions match your search." : "No sessions yet."),
			];
		}
	...
	private renderRow(
		...
		if (isAgentsViewSectionHeadingRow(row)) return this.renderSectionHeadingRow(row, width, selected);
	...
	private renderHints(width: number): string {
		...
		const hints = [
			...
			selectedAgent
				? `${keyText("app.agents.delete")} ${selectedRow?.activitySection === "inactive" ? "delete" : "stop/deactivate"}`
				: undefined,
			selectedSubagent
				? `${keyText("app.agents.delete")} ${selectedRow.activitySection === "running" ? "stop" : "delete"}`
				: undefined,
	...
	private formatRowIcon(section: "running" | "idle" | "inactive", icon: string): string {
		switch (section) {
			case "running":
...
function countRowsBySection(rows: readonly AgentsViewSessionRow[]): Record<AgentsViewSection, number> {
	...
	return {
		...
		running: agents.filter((row) => row.displaySection === "running").length,
		idle: agents.filter((row) => row.displaySection === "idle").length,
		inactive: agents.filter((row) => row.displaySection === "inactive").length,
	};
}
...
function hasLiveWork(row: AgentsViewSessionRow): boolean {
...
export function buildCompactAgentsViewLayout(rows: readonly AgentsViewRow[], width = 120): AgentsViewUsageLayout {
	const sessions = rows.filter((row): row is AgentsViewSessionRow => row.kind === "agent" || row.kind === "subagent");
...
// rows carry no level).
function formatSessionModel(row: AgentsViewSessionRow): string {
