...
export class AgentDaemon {
	...
	private async passiveRlmSubagentEntryForEdge(
		...
		if (!registryRead) {
			registryRead = this.readLegacyRlmSubagentRegistry(registryPath, false, () => {
	...
	private async deleteRlmHeartbeatForState(
		state: ActiveSessionState,
		id: string,
	): Promise<AgentRlmHeartbeatCancellationReceipt | undefined> {
		const cancellation = await this.cronStore.deleteRlmHeartbeat(state.activeSessionId, id);
		if (cancellation) {
	...
	private async streamWorkerSnapshot(
		...
		try {
			...
			if (!snapshotDelivered) return false;
			onDelivered?.();
...
function snapshotTransferId(snapshot: DaemonSessionSnapshot): string {
	...
	return createSnapshotTransferId(snapshot.activeSessionId, cursor.generation, cursor.sequence);
}
