...
import {
	...
	DAEMON_STABLE_TARGET_FOLLOW_UP_COMPATIBILITY,
...
import {
	...
	type NormalizedStableSessionTarget,
...
// Fresh catalogs younger than this are reused when freshness is allowed.
const SUMMARY_FRESHNESS_MS = 2000;
const SUMMARY_REHYDRATION_INITIAL_DELAY_MS = 5000;
const SUMMARY_REHYDRATION_MAX_DELAY_MS = 30_000;
...
interface ResidentWorker {
	...
	/**
	 * The adopted worker's own `daemon_hello`, retained as its capability proof.
	 * Never inferred from the supervisor's own protocol/schema: a worker left
	 * behind by an older binary must be able to prove schema 24 for itself.
	 */
	hello?: DaemonHello;
...
export class DaemonSupervisor {
	...
	private attachLeaseRegistry?: AttachLeaseRegistry;
	private get attachWaits(): AttachWaitRegistry {
		this.attachWaitRegistry ??= new AttachWaitRegistry();
		return this.attachWaitRegistry;
	}
	private get attachLeases(): AttachLeaseRegistry {
		this.attachLeaseRegistry ??= new AttachLeaseRegistry();
		return this.attachLeaseRegistry;
	}
	...
	private scheduledWakeDrain?: Promise<void>;
	private scheduledWakeRevision = 0;
	...
	private scheduleScheduledSessionWakeRecompute(): void {
		...
		if (this.scheduledWakeDrain || this.scheduledWakeRecompute) {
	...
	private async collectPassiveScheduledJobs(
		...
		rootKey?: string,
		familySnapshot?: { infos?: SessionInfo[] },
	): Promise<PassiveScheduledJob[]> {
		// A subtree scan must not replace the global catalog snapshot.
		const epoch = rootKey === undefined ? this.claimPassiveScheduledJobsEpoch() : undefined;
		const rows = await this.scanPassiveScheduledJobs(rootKey, familySnapshot);
		if (epoch !== undefined) this.storePassiveScheduledJobs(epoch, rows);
	...
	private async scanPassiveScheduledJobs(
		rootKey?: string,
		familySnapshot?: { infos?: SessionInfo[] },
	): Promise<PassiveScheduledJob[]> {
		...
		const infos = familySnapshot?.infos ?? (await this.rlmSpawnLedger().family(SESSION_SCHEDULED_JOBS_FILENAME));
		if (familySnapshot) familySnapshot.infos = infos;
		...
		for (const info of infos) {
			...
			if (rootKey !== undefined) {
				const root = this.uncoveredRootForInfo(info, infoByPath);
				if (root === undefined || canonicalSessionPath(root) !== rootKey) continue;
			}
	...
	private async scheduledWakeCandidates(familySnapshot?: { infos?: SessionInfo[] }): Promise<PassiveScheduledJob[]> {
		const cached = this.passiveScheduledJobs;
		if (cached && !this.ephemeralCancelIntentsPending) return activeScheduledJobs(cached.rows);
		return activeScheduledJobs(await this.collectPassiveScheduledJobs(false, undefined, familySnapshot));
	}
	...
	private async recomputeScheduledSessionWake(): Promise<void> {
		...
		if (this.scheduledWakeDrain) {
			this.scheduledWakeRecomputeQueued = true;
			return;
		}
		const revision = this.scheduledWakeRevision;
		const candidates = await this.scheduledWakeCandidates();
		this.clearScheduledWakeTimer();
		if (this.shuttingDown || this.updateRestartPhase !== undefined) return;
		// A drain can start and finish while the ledger is loading. Discard that stale scan.
		if (this.scheduledWakeDrain || revision !== this.scheduledWakeRevision) {
			this.scheduledWakeRecomputeQueued = true;
			return;
		}
		const now = Date.now();
	...
	private wakeDueScheduledSessions(now = Date.now()): Promise<void> {
		if (this.scheduledWakeDrain) return this.scheduledWakeDrain;
		this.clearScheduledWakeTimer();
		// Disarmed during update-restart preparation; the phase transition or next boot re-arms once.
		if (this.shuttingDown || this.updateRestartPhase !== undefined) return Promise.resolve();
		this.scheduledWakeRevision = (this.scheduledWakeRevision ?? 0) + 1;
		const startedAt = Date.now();
		const wakeNow = () => now + Math.max(0, Date.now() - startedAt);
		const backedOff = (rootKey: string): boolean => {
			const failedAt = this.scheduledWakeFailures.get(rootKey);
			return failedAt !== undefined && failedAt + SCHEDULED_WAKE_RETRY_MS > wakeNow();
		};
		const isDue = (job: AgentCronJob): boolean => {
			if (job.status !== "active" || job.nextRunAt === undefined) return false;
			const runAt = Date.parse(job.nextRunAt);
			return Number.isFinite(runAt) && runAt <= wakeNow();
		};
		// Publish before collection or worker creation can broadcast another wake request.
		const drain = Promise.resolve()
			.then(async () => {
				const due = new Map<string, string>();
				// Topology is a pass snapshot; per-root artifacts and resident ownership stay fresh.
				const familySnapshot: { infos?: SessionInfo[] } = {};
				for (const { rootSessionFile, job } of await this.scheduledWakeCandidates(familySnapshot)) {
					if (isDue(job)) due.set(canonicalSessionPath(rootSessionFile), rootSessionFile);
				}
				for (const [rootKey, sessionPath] of due) {
					if (this.shuttingDown || this.updateRestartPhase !== undefined) return;
					if (backedOff(rootKey)) continue;
					try {
						// A previous launch awaited startup. Re-read only this root's artifacts:
						// it may now be resident, paused, cancelled, or no longer due.
						const current = await this.collectPassiveScheduledJobs(false, rootKey, familySnapshot);
						if (this.shuttingDown || this.updateRestartPhase !== undefined) return;
						if (
							!current.some(
								({ rootSessionFile, job }) => canonicalSessionPath(rootSessionFile) === rootKey && isDue(job),
							)
						) {
							// Disk revalidation supersedes the cached due candidate. Rescan before
							// re-arming so a paused or rescheduled job cannot spin a zero-delay wake.
							this.invalidatePassiveScheduledJobs();
							continue;
						}
						if (backedOff(rootKey) || this.findWorkerBySessionFile(sessionPath)) continue;
			...
			.finally(() => {
				if (this.scheduledWakeDrain === drain) this.scheduledWakeDrain = undefined;
				this.scheduledWakeRecomputeQueued = false;
				this.scheduleScheduledSessionWakeRecompute();
			});
		this.scheduledWakeDrain = drain;
		return drain;
	}
	...
	private handleConnection(socket: Socket): void {
		...
		const cleanup = () => {
			...
			cleaned = true;
			this.attachWaits.cancelClient(client);
	...
	private async handleLine(client: DaemonSocketClient, line: string): Promise<void> {
		let attachScope: AttachWaitScope | undefined;
		let attachOutcome: "completed" | "failed" = "failed";
		try {
			try {
				this.assertSupervisorServing();
			} catch (error) {
			...
			if (command.type === "attach") {
				try {
					attachScope = this.attachWaits.start(client, command.id ?? randomUUID(), {
						activeSessionId: command.activeSessionId,
						clientId: client.id,
			...
			} else if (command.type === "cancel_attach") {
				this.attachWaits.cancel(client, command.requestId, command.activeSessionId);
			} else if (command.type === "detach") {
				if (command.activeSessionId) this.attachWaits.cancelSession(client, command.activeSessionId);
				else this.attachWaits.cancelClient(client);
			}
			...
			try {
				if (attachScope) await attachScope.wait("admission", () => this.ready);
				else await waitForPromptAdmission(this.ready, parsedAdmission?.controller.signal);
			} catch (error) {
			...
			try {
				if (attachScope) await attachScope.wait("ownership", () => this.assertServingCurrentOwnership());
				else await waitForPromptAdmission(this.assertServingCurrentOwnership(), parsedAdmission?.controller.signal);
			} catch (error) {
				if (parsedAdmission) this.deletePromptAdmission(parsedAdmission);
				this.write(client, failure(command.id, command.type, error));
				return;
			...
			const journalIdentity =
				envelopeClientId && command.id && mutation
			...
			try {
				const response = await this.handleCommand(client, command, cancellationAdmission, attachScope);
				if (response) {
					if (journalIdentity) {
						await this.assertCurrentOwnership();
					...
					attachOutcome = response.success ? "completed" : "failed";
				}
			} catch (error) {
			...
			if (!attachScope?.transferred) attachScope?.finish(attachOutcome);
		}
	}
	...
	private async handleCommand(
		...
		switch (command.type) {
			...
			case "attach": {
				...
				const ownedScope = attachScope === undefined;
				const scope =
					attachScope ??
					this.attachWaits.start(client, command.id ?? randomUUID(), {
						activeSessionId: command.activeSessionId,
						clientId: client.id,
						timeoutMs: command.timeoutMs,
						log: (line) => this.log(line),
					});
				let attached: WorkerAttachData | undefined;
				try {
					attached = await this.attachClient(client, command, scope);
					scope.check();
					if (attached.result.client.capabilities.includes("chunked_snapshot")) {
						const transcript = attached.transcript;
						if (!transcript) throw new Error("Session worker did not provide a snapshot transcript");
						...
						void scope
							...
							.then(
								(delivered) => {
									if (delivered) current.commitAttachment?.();
									else current.releaseAttachment?.();
									scope.finish(delivered ? "completed" : "failed");
								},
								(error: unknown) => {
									current.releaseAttachment?.();
									scope.finish("failed");
									if (!(error instanceof AttachCancelledError))
										this.log(
					...
					attached.commitAttachment?.();
					if (ownedScope) scope.finish();
					return success(command.id, "attach", attached.result);
				} catch (error) {
					attached?.releaseTranscript?.();
					attached?.releaseAttachment?.();
					scope.finish("failed");
					throw error;
				}
			}
		...
		// Capability-gated durable addressing. Everything else about follow_up,
		// including the ordinary active-session route below, is untouched.
		if (command.type === "follow_up" && command.stableTarget !== undefined) {
			return this.handleStableTargetFollowUp(client, command);
		}
	...
	private async createOrReuseWorker(clientId: string, command: DaemonCreateCommand): Promise<ResidentWorker> {
		...
		// Opening-reservation invariant: publish the reservation for `key` in this
		// synchronous turn, before the opening body executes a single statement.
		// `Promise.resolve().then(...)` defers the body to a microtask, so stale
		// catalog hydration, worker rehydration, name validation and the launch all
		// run while the reservation is already visible. A rival for the same key
		// therefore always joins this open (or receives its typed refusal) and can
		// never launch a second worker, replace one, or adopt the wrong session,
		// no matter how slow or blocked the hydration inside the body is.
		const opening = Promise.resolve().then(() => this.openReservedWorker(createCommand, ownerClientId));
	...
	/**
	 * Runs entirely inside the published opening reservation for the create key.
	 * Every await here (stale-catalog hydration, stale-registration reclaim, worker
	 * rehydration/recovery, saved-name validation, launch) is covered by the
	 * reservation, so ownership of the key is authoritative until it is committed
	 * or released by {@link createOrReuseWorker}'s `finally`.
	 */
	private async openReservedWorker(
		createCommand: DaemonCreateCommand,
		ownerClientId: string | undefined,
	): Promise<ResidentWorker> {
		if (createCommand.sessionPath) {
			// A stale resident catalog can still hide an already-resident owner of
			// this saved path; hydrate before deciding, under the reservation.
			await this.refreshRelevantStaleWorkerSummaries(createCommand.sessionPath);
			const existing = this.findWorkerBySessionFile(createCommand.sessionPath);
			if (
				existing &&
				!(await this.reclaimStaleWorkerRegistration(existing, createCommand.launchEnv !== undefined))
			) {
				return this.reuseWorkerForCreate(existing, ownerClientId, createCommand.sessionPath);
			}
		}
	...
	/**
	 * A stale worker catalog can hide an already-resident saved descendant. Walk
	 * the bounded spawn ledger to its root, refresh only matching resident
	 * workers, and fail transiently when ownership still cannot be established.
	 * Launching in that state could create a second worker for the same child.
	 */
	private async refreshRelevantStaleWorkerSummaries(sessionPath: string): Promise<void> {
		if (![...this.workers.values()].some((worker) => worker.summariesStale === true)) {
			return;
		}
		const targetPath = canonicalSessionPath(sessionPath);
		let edges: Awaited<ReturnType<RlmSpawnLedger["edges"]>>;
		try {
			edges = await this.rlmSpawnLedger().edges();
		} catch (error) {
			throw new Error(
				`Session "${sessionPath}" cannot be safely opened while resident worker catalogs are stale: ${String(error)}`,
			);
		}
		const parentByChild = new Map(
			edges.map((edge) => [canonicalSessionPath(edge.child), canonicalSessionPath(edge.parent)]),
		);
		const ancestorPaths = new Set<string>([targetPath]);
		let ancestorPath = targetPath;
		while (true) {
			const parentPath = parentByChild.get(ancestorPath);
			if (!parentPath || ancestorPaths.has(parentPath)) {
				break;
			}
			ancestorPaths.add(parentPath);
			ancestorPath = parentPath;
		}
		const isRelevant = (worker: ResidentWorker): boolean => {
			const root = worker.summaries.get(worker.descriptor.rootActiveSessionId);
			return [worker.descriptor.sessionFile, worker.descriptor.createCommand.sessionPath, root?.sessionFile].some(
				(path) => path !== undefined && ancestorPaths.has(canonicalSessionPath(path)),
			);
		};
		const relevantWorkers = [...this.workers.values()].filter(
			(worker) => worker.summariesStale === true && isRelevant(worker),
		);
		if (relevantWorkers.length === 0) {
			return;
		}
		await Promise.allSettled(relevantWorkers.map((worker) => this.refreshWorkerSummaries(worker)));
		if (this.findWorkerBySessionFile(sessionPath)) {
			return;
		}
		const unresolved = [...this.workers.values()].filter(
			(worker) =>
				this.workers.get(worker.descriptor.workerId) === worker &&
		...
		if (unresolved.length > 0) {
			throw new Error(
				`Session "${sessionPath}" cannot be safely opened while its resident worker catalog is stale; retry shortly`,
			);
		}
	}
	...
	private async reuseWorkerForCreate(
		...
		if (this.canRetryFailedWorker(worker)) {
			...
			} else if (worker.summaryRehydration) {
				await worker.summaryRefresh?.catch(() => undefined);
				if (!worker.summaries.has(worker.descriptor.rootActiveSessionId)) {
					await this.seedWorkerRootSummary(worker);
				}
	...
	private async launchWorker(
		...
		try {
			...
			await this.refreshWorkerSummaries(worker);
			if (!worker.summaries.has(rootActiveSessionId)) {
				throw new Error("Session worker omitted its root session during startup");
			}
	...
	private async seedWorkerRootSummary(worker: ResidentWorker): Promise<SessionSummary> {
		...
		const response = await client.request({ type: "get_state", activeSessionId: rootActiveSessionId }, 5000);
		if (!response.success) {
			throw deserializeDaemonError(response);
		}
		if (!isSessionSummary(response.data)) {
			throw new Error("Session worker returned an invalid root state response");
		}
		const summary = response.data;
		if ((summary.activeSessionId ?? summary.id) !== rootActiveSessionId) {
			throw new Error("Session worker did not preserve its assigned root session id");
		}
		if (
			this.shuttingDown ||
			this.workers.get(worker.descriptor.workerId) !== worker ||
			...
			throw new Error("Session worker changed during root state refresh");
		}
	...
	private async adoptOrRecoverWorker(worker: ResidentWorker): Promise<void> {
		...
		try {
			...
			try {
				...
				// A slow or root-omitting catalog must not fail adoption of a live
				// worker: seed the cheap root state, keep prior summaries, mark
				// stale, and rehydrate the full catalog in the background.
				try {
					await this.seedWorkerRootSummary(worker);
				} catch (seedError) {
					this.log(`Adopted worker ${worker.descriptor.workerId} with stale summaries: ${String(seedError)}`);
				}
				this.log(
					`Adopted worker ${worker.descriptor.workerId} with a stale catalog; rehydrating in the background: ${String(refreshError)}`,
				);
				this.scheduleSummaryRehydration(worker);
			}
					...
					if (identityCompatible) {
						try {
							...
							try {
								...
								// A slow catalog must not turn recovery of a live worker into a
								// kill or a replacement: seed the root, mark stale, rehydrate.
								try {
									await this.seedWorkerRootSummary(worker);
								} catch (seedError) {
									this.log(
										`Recovered worker ${worker.descriptor.workerId} with stale summaries: ${String(seedError)}`,
									);
								}
								this.log(
									`Recovered worker ${worker.descriptor.workerId} with a slow catalog; rehydrating in the background: ${String(refreshError)}`,
								);
								this.scheduleSummaryRehydration(worker);
							}
	...
	private canRehydrateWorkerSummaries(worker: ResidentWorker): boolean {
		return (
			!this.shuttingDown &&
			this.workers.get(worker.descriptor.workerId) === worker &&
			!this.isWorkerStopping(worker) &&
	...
	private scheduleSummaryRehydration(worker: ResidentWorker): void {
		if (worker.summaryRehydration) {
			return;
		}
		const rehydration = this.rehydrateWorkerSummaries(worker)
			.catch((error) => {
				this.log(
					`Summary rehydration stopped unexpectedly for worker ${worker.descriptor.workerId}: ${String(error)}`,
				);
			})
			.finally(() => {
				if (worker.summaryRehydration === rehydration) {
					worker.summaryRehydration = undefined;
				}
			});
		worker.summaryRehydration = rehydration;
	}

	private async rehydrateWorkerSummaries(worker: ResidentWorker): Promise<void> {
		let retryDelayMs = SUMMARY_REHYDRATION_INITIAL_DELAY_MS;
		while (this.canRehydrateWorkerSummaries(worker)) {
			try {
				await this.assertRecoveryAllowed();
				await this.refreshWorkerSummaries(worker);
				return;
			} catch (error) {
				if (isSupervisorRecoveryCancelled(error) || !this.canRehydrateWorkerSummaries(worker)) {
					return;
				}
				this.log(`Could not rehydrate worker ${worker.descriptor.workerId} summaries: ${String(error)}`);
			}
			if (!this.canRehydrateWorkerSummaries(worker)) {
				return;
			}
			await unrefDelay(retryDelayMs);
			retryDelayMs = Math.min(retryDelayMs * 2, SUMMARY_REHYDRATION_MAX_DELAY_MS);
		}
	}

	private refreshWorkerSummaries(worker: ResidentWorker, allowFresh = false): Promise<void> {
		// Bounded freshness: an authoritative recent catalog is reused instead of
		// re-walked, and concurrent callers join one in-flight pull instead of
		// starting parallel roster walks (fork summary-freshness law, carried over
		// the roster-ledger merge).
		if (
			allowFresh &&
			worker.summariesStale !== true &&
			worker.summariesRefreshedAt !== undefined &&
			Date.now() - worker.summariesRefreshedAt < SUMMARY_FRESHNESS_MS
		) {
		...
		const refresh = this.pullWorkerSummaries(worker, false).then(
			() => {
		...
		return refresh.finally(() => {
			if (worker.summaryRefresh === refresh) {
				worker.summaryRefresh = undefined;
			}
		});
	}

	private async pullWorkerSummaries(
		worker: ResidentWorker,
		recovery: boolean,
		fillGaps = true,
		retried = false,
		...
		if (fillGaps && (worker.rosterEpoch ?? 0) !== epochAtStart && !retried) {
			return this.pullWorkerSummaries(worker, recovery, fillGaps, true);
		}
		...
		// A catalog that drops the worker's own root is never accepted as truth:
		// the prior authoritative catalog stays in place and the worker goes stale.
		if (!root) {
			throw new Error("Session worker omitted its root session during catalog hydration");
		}
		worker.summaries = nextSummaries;
		worker.summariesRefreshedAt = Date.now();
		if (fillGaps) {
	...
	private scheduleRosterRepairPull(worker: ResidentWorker): void {
		...
		worker.rosterRepairPull = this.refreshWorkerSummaries(worker)
			.catch((error: unknown) =>
	...
	/**
	 * Route a `follow_up` addressed by durable session identity.
	 *
	 * A retained RLM child that the daemon has passivated still publishes its
	 * durable sessionId and sessionFile on its owning worker's roster row, but it
	 * no longer owns an ephemeral activeSessionId, so the ordinary route answers
	 * "Unknown active session" and the terminal reminder is lost. This branch
	 * resolves the roster row by durable coordinates only — never by suffix, name,
	 * newest row, root, or Board — and then forwards the ordinary follow_up
	 * command to the owning worker, which hydrates the child through the existing
	 * passive-subagent seam and queues into the normal waking follow-up lane.
	 *
	 * Wire-change class: capability-gated (`stable_target_follow_up`). Callers
	 * without the capability are refused by DaemonClient before the request is
	 * written, so an unsupported semantic request is never sent as if it worked.
	 */
	private async handleStableTargetFollowUp(
		client: DaemonSocketClient,
		command: Extract<DaemonCommand, { type: "follow_up" }>,
	): Promise<DaemonResponse> {
		const target = normalizeStableSessionTarget(command.stableTarget ?? {});
		let matches = this.matchStableTargetSummaries(client, target);
		if (matches.length === 0) {
			// One bounded authoritative refresh, exactly as findWorker does, so a
			// target that only just passivated is not reported absent. No polling
			// loop, and no second roster walk: refreshWorkerSummaries returns any
			// hydration already in flight, so a request arriving mid-rehydration
			// joins that single-flight and then matches its authoritative result.
			await this.refreshStableTargetCatalogs(client);
			matches = this.matchStableTargetSummaries(client, target);
		}
		if (matches.length > 1) {
			throw new StableFollowUpTargetError(
				"ambiguous",
				target,
				`Stable follow-up target matches ${matches.length} retained sessions`,
			);
		}
		const match = matches[0];
		if (!match) {
			// A terminal "not_found" is a promise that the target does not exist,
			// and the public helper treats it as terminal. Any worker whose roster
			// could not be authoritatively established could still own the target,
			// so that state is a retriable target_unavailable rather than a
			// terminal refusal manufactured from an empty or stale snapshot.
			const gaps = this.stableTargetOwnershipGaps(client);
			if (gaps.length > 0) {
				throw new StableFollowUpTargetError(
					"target_unavailable",
					target,
					`Stable follow-up target ownership could not be established: ${gaps.join("; ")}`,
				);
			}
			const visible = [...this.workers.values()]
				.filter((worker) => this.isWorkerAccessibleToClient(client, worker))
				.flatMap((worker) => [...worker.summaries.values()]);
			throw new StableFollowUpTargetError(
				isStableTargetIdentityMismatch(visible, target) ? "identity_mismatch" : "not_found",
				target,
				isStableTargetIdentityMismatch(visible, target)
		...
		assertStableFollowUpTargetUsable(match.summary, target);
		if (this.isWorkerStopping(match.worker) || !match.worker.client) {
			throw new StableFollowUpTargetError(
				"target_unavailable",
				target,
				`Session worker for the stable follow-up target is ${this.isWorkerStopping(match.worker) ? "stopping" : "not connected"}`,
			);
		}
		// Mixed-version fail-closed. The supervisor's own schema says nothing about
		// a worker it adopted from an older binary, so the owning worker must have
		// proven protocol 7 / schema 24 / stable_target_follow_up in its own hello
		// before it may be sent a stableTarget. A worker that cannot is refused as
		// retriable and left to the controlled recovery path; it is never silently
		// downgraded to active-id-only delivery.
		if (!this.workerProvesStableTargetFollowUp(match.worker)) {
			throw new StableFollowUpTargetError(
				"target_unavailable",
				target,
				`Session worker ${match.worker.descriptor.workerId} has not proven the ${DAEMON_STABLE_TARGET_FOLLOW_UP_COMPATIBILITY.capability} capability required to receive a stable follow-up target`,
			);
		}
		// Forward the resolved durable coordinates, not the caller's partial ones:
		// the worker re-resolves conjunctively against its own authoritative state,
		// which closes the roster-read-to-delivery race without a second round trip.
		return this.forwardToWorker(match.worker, {
			...command,
			activeSessionId: match.summary.activeSessionId ?? match.summary.sessionId,
			stableTarget: { sessionId: match.summary.sessionId, sessionFile: match.summary.sessionFile },
		});
	}

	/**
	 * Bring every client-visible worker's roster to an authoritative state.
	 *
	 * `refreshWorkerSummaries` is single-flight per worker, so a worker already
	 * hydrating in the background is *joined* rather than walked a second time,
	 * and a worker sleeping between rehydration attempts is advanced by the one
	 * walk started here (which its own loop then joins). Failures are settled,
	 * not swallowed: the resulting per-worker staleness is what
	 * `stableTargetOwnershipGaps` reads to keep a failure from masquerading as a
	 * terminal absence.
	 */
	private async refreshStableTargetCatalogs(client: DaemonSocketClient): Promise<void> {
		await Promise.allSettled(
			[...this.workers.values()]
				...
				.map((worker) => this.refreshWorkerSummaries(worker)),
		);
	}

	/**
	 * Why no client-visible worker can currently be trusted to have disproven the
	 * target. Empty means every visible roster is authoritative, which is the only
	 * state in which a terminal refusal may be reported.
	 */
	private stableTargetOwnershipGaps(client: DaemonSocketClient): string[] {
		const gaps: string[] = [];
		for (const worker of this.workers.values()) {
			if (!this.isWorkerAccessibleToClient(client, worker)) continue;
			const workerId = worker.descriptor.workerId;
			if (this.isWorkerStopping(worker)) {
				gaps.push(`worker ${workerId} is stopping`);
			} else if (!worker.client) {
				gaps.push(`worker ${workerId} is not connected`);
			} else if (worker.summariesStale === true) {
				gaps.push(`worker ${workerId} has a stale session catalog`);
			} else if (worker.summaryRefresh !== undefined || worker.summaryRehydration !== undefined) {
				gaps.push(`worker ${workerId} is still hydrating its session catalog`);
			} else if (!worker.summaries.has(worker.descriptor.rootActiveSessionId)) {
				gaps.push(`worker ${workerId} omitted its root session from its catalog`);
			} else if (!this.workerProvesStableTargetFollowUp(worker)) {
				// Coordinated adoption: an unproven resident worker could own the
				// target and could not be forwarded to, so the absence is unproven.
				gaps.push(`worker ${workerId} has not proven the stable follow-up capability`);
			}
		}
		return gaps;
	}
	...
	/**
	 * Whether this worker's own greeting proves it implements stable-target
	 * follow-ups. Read from the connection's retained hello, so a worker with no
	 * live connection or no greeting is never credited.
	 */
	private workerProvesStableTargetFollowUp(worker: ResidentWorker): boolean {
		if (!worker.client) {
			return false;
		}
		return daemonHelloMeetsCompatibility(
			worker.client.hello ?? worker.hello,
			DAEMON_STABLE_TARGET_FOLLOW_UP_COMPATIBILITY,
		);
	}

	/** Every roster row satisfying every supplied durable coordinate. Conjunctive. */
	private matchStableTargetSummaries(
		client: DaemonSocketClient,
		...
		for (const worker of this.workers.values()) {
			if (!this.isWorkerAccessibleToClient(client, worker)) continue;
			for (const summary of worker.summaries.values()) {
				if (matchesStableSessionTarget(summary, target)) matches.push({ worker, summary });
			}
		}
		return matches;
	}
	...
	private async findWorker(
		...
		const descriptorCandidates = [...this.workers.values()].filter(
			(worker) =>
		...
		for (const worker of descriptorCandidates) {
			try {
				const summary = await this.seedWorkerRootSummary(worker);
				if ((summary.activeSessionId ?? summary.id) === selector || summary.sessionId === selector) {
					confirmed.push({ worker, summary });
				}
			} catch {
				// A descriptor is only a hint; route only after worker-side confirmation.
			}
		}
	...
	private async forwardToWorker(
		...
		if (command.type !== "kill" && this.canRetryFailedWorker(worker)) {
			await this.retryWorkerRecovery(worker);
		} else if (command.type !== "kill" && worker.recovery) {
	...
	private async stopKilledRootWorker(worker: ResidentWorker, command: DaemonCommand): Promise<DaemonResponse> {
		// Hold stop ownership across the whole kill+stop window so a concurrent
		// eviction/monitor path cannot start a competing stop of this worker while
		// the bounded kill acknowledgement is still outstanding.
	...
	private async attachClient(
		...
		scope?: AttachWaitScope,
	): Promise<WorkerAttachData> {
		scope?.check();
		const wait = <T>(phase: string, work: () => Promise<T>): Promise<T> => (scope ? scope.wait(phase, work) : work());
		...
		if (descriptorWorker) {
			...
			if (descriptorWorker.descriptor.ownerClientId !== undefined) {
				...
				if (!descriptorWorker.client || descriptorWorker.descriptor.lifecycle !== "ready") {
					...
					await wait("worker_recovery", () => this.retryWorkerRecovery(descriptorWorker));
				}
			} else if (this.canRetryFailedWorker(descriptorWorker)) {
				await wait("worker_recovery", () => this.retryWorkerRecovery(descriptorWorker));
			}
		}
		const match = await wait("worker_lookup", () => this.findWorkerForClient(client, command.activeSessionId));
		scope?.setWorker(match.worker.descriptor.workerId, match.summary.activeSessionId ?? match.summary.id);
		...
		if (match.worker !== descriptorWorker && this.canRetryFailedWorker(match.worker)) {
			...
			await wait("worker_recovery", () => this.retryWorkerRecovery(match.worker));
		}
		...
		if (duplicateValidation) {
			await wait("validation", () => duplicateValidation.promise);
		}
		if (command.clientId) {
		...
		if (!result) {
			...
			while (!result) {
				...
				if (!loading) {
					...
					const loadCapabilities = capabilities.has("chunked_snapshot")
						? (["attach_snapshot", "event_sequence", "slim_attach", "chunked_snapshot"] as const)
						: (["attach_snapshot", "event_sequence", "slim_attach"] as const);
					const loadEnv = command.env ?? collectDaemonClientEnv();
					loading = (async () => {
				...
				try {
					result = await wait("worker_snapshot", () => loading!);
				} catch (error) {
		...
		if (capabilities.has("chunked_snapshot")) {
			while (true) {
				...
				if (validation) {
					await wait("validation", () => validation.promise);
					continue;
				}
		...
		scope?.check();
		if (scope && client.socket.destroyed) throw new AttachCancelledError("disconnected");
		const releaseTranscript = transcript?.retain();
		const lease = scope
			? this.attachLeases.acquire(client, activeSessionId, {
					has: () => client.attachedActiveSessionIds.has(activeSessionId),
					add: () => client.attachedActiveSessionIds.add(activeSessionId),
					delete: () => {
						client.attachedActiveSessionIds.delete(activeSessionId);
						void this.syncWorkerExtensionUi(activeSessionId);
					},
			...
			: undefined;
		if (!lease) client.attachedActiveSessionIds.add(activeSessionId);
		const commitAttachment = () => {
			client.capabilities = capabilities;
			client.supportsExtensionUi = capabilities.has("extension_ui");
			lease?.commit();
			void this.syncWorkerExtensionUi(activeSessionId);
		};
		if (!scope) commitAttachment();
		try {
			...
			return {
				...
				releaseTranscript,
				commitAttachment,
				releaseAttachment: lease?.release,
			};
		} catch (error) {
			releaseTranscript?.();
			lease?.release();
			if (!lease && !wasAttached) {
				client.attachedActiveSessionIds.delete(activeSessionId);
			}
			throw error;
		}
	...
	private async streamSnapshot(
		...
		const snapshotSignal = client.snapshotTransferAbortControllers?.get(result.activeSessionId)?.signal;
		const signal =
			attachSignal && snapshotSignal
				? AbortSignal.any([attachSignal, snapshotSignal])
				: (attachSignal ?? snapshotSignal);
	...
	private async writeSnapshotBuffer(
		...
		return new Promise<boolean>((resolveDrain) => {
			...
			if (signal?.aborted || client.socket.destroyed) onClose();
		});
	}

	private detachClient(client: DaemonSocketClient, activeSessionId?: string): void {
		if (activeSessionId) this.attachWaits.cancelSession(client, activeSessionId);
		else this.attachWaits.cancelClient(client);
	...
	private async drainClientCatchups(client: DaemonSocketClient): Promise<void> {
		...
		for (let index = 0; index < pending.length; index++) {
			...
			if (!client.attachedActiveSessionIds.has(activeSessionId)) continue;
			const scope = this.attachWaits.start(client, `catchup:${randomUUID()}`, {
				activeSessionId,
				clientId: client.id,
				log: (line) => this.log(line),
			});
			...
			try {
				attached = await this.attachClient(
					client,
					{
						type: "attach",
						activeSessionId,
				...
				if (client.capabilities.has("chunked_snapshot")) {
					...
					delivered = await this.streamSnapshot(
						client,
						attached.worker,
				...
				releaseTranscript?.();
				if (
					(!(error instanceof AttachCancelledError) || error.reason === "deadline") &&
					!client.socket.destroyed &&
				...
				client.deferredSessionPayloadsDropped.add(activeSessionId);
				this.discardDeferredSessionPayloads(client, activeSessionId);
				if (!(error instanceof AttachCancelledError) || error.reason === "deadline")
				...
				if (delivered && !scope.signal.aborted) attached?.commitAttachment?.();
				else attached?.releaseAttachment?.();
				scope.finish(delivered ? "completed" : "failed");
			}
		}
	...
	private fenceSupervisorSocket(): void {
		for (const client of this.clients) this.attachWaits.cancelClient(client);
		try {
			this.server?.close();
		} catch {
