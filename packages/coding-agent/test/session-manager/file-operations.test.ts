...
interface FileLinesModule {
	...
	readLinesAsBuffers(filePath: string, range?: { start?: number; end?: number }): AsyncGenerator<Buffer>;
}
...
describe("readSessionInfo", () => {
	...
	it("reads only appended JSONL bytes after a large completed prefix", async () => {
		...
		expect(fileLineMocks.readLinesAsBuffers).toHaveBeenCalledWith(file, { start, end: expect.any(Number) });
	});
	...
	it("falls back to byte zero when a same-inode rewrite regrows beyond the cached size", async () => {
		...
		expect(fileLineMocks.readLinesAsBuffers).toHaveBeenCalledWith(file, { start: 0, end: expect.any(Number) });
	});
	...
	it("does not cache a transient stream failure and clears its concurrent flight", async () => {
		...
		await expect(Promise.all([first, second])).resolves.toEqual([
			null,
			expect.objectContaining({ firstMessage: "recovered" }),
		]);
...
describe("readSessionInfo incremental scans", () => {
	...
	it("rescans a rewritten prefix and folds the completed torn tail once", async () => {
		const file = join(tempDir, "incremental.jsonl");
		const torn = line(msg("m2", "m1", "assistant", "answer"));
		writeFileSync(
			file,
			line(header) +
				line(msg("m1", null, "user", "original question")) +
				line(msg("padding", "m1", "assistant", "x".repeat(9000))) +
				torn.slice(0, 20),
		);
		...
		// the retained boundary check must invalidate the old accumulator.
		...
		expect(info?.firstMessage).toBe("modified question");
	});
