...
import type { AttachWaitScope } from "../src/modes/daemon/attach-wait.js";
...
describe("deferred session frames during snapshot streams", () => {
	...
	it.each(["lookup", "stream"])("retries catch-up after a deadline during %s without newer events", async (phase) => {
		vi.useFakeTimers();
		const supervisor = new DaemonSupervisor(join(tmpdir(), "deadline-catchup.sock"), {
			defaultSessionConfig: { agentDir: "/tmp", cwd: "/tmp" },
			descriptorDir: join(tmpdir(), "deadline-catchup-state"),
		});
		const socket = new PassThrough();
		const client = socketClient(socket);
		const written: DaemonOutbound[] = [];
		...
		const worker = {
			descriptor: { workerId: "deadline-worker", lifecycle: "ready", pid: 987_654 },
			...
			stopRevision: 0,
		} satisfies SupervisorWorkerHarness;
		const internals = supervisor as unknown as {
			clients: Set<DaemonSocketClient>;
			attachClient(
				client: DaemonSocketClient,
		...
		const attach = vi.spyOn(internals, "attachClient").mockImplementation(async (_client, _command, scope) => {
			if (phase === "lookup") await scope.wait("lookup", () => new Promise<never>(() => {}));
			return { worker, result: streamedResult(), transcript };
		});
		try {
			client.catchupActiveSessionIds?.add(activeSessionId);
			const pending = internals.catchUpClient(client);
			...
			attach.mockResolvedValue({ worker, result: streamedResult(), transcript });
			await vi.advanceTimersByTimeAsync(1000);
			expect(attach).toHaveBeenCalledTimes(2);
			expect(written.at(-1)?.type).toBe("session_snapshot_end");
			...
			expect(written.at(-1)?.type).toBe("session_event");
		} finally {
			clearTimeout(client.catchupRetryTimer);
			socket.destroy();
			transcript.dispose();
	...
	it.each(supervisorScenarios)("supervisor: replay (%s)", async (scenario) => {
		...
		else if (reattach || catchup) expect(error).toBeUndefined();
		else expect(error).toBe(!closed || overflow);
