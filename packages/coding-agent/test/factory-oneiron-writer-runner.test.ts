	...
	test.each([false, true])(
		...
		async (competitor) => {
			...
			try {
				...
				writeFileSync(
					fixture,
					`import {writeFileSync} from 'node:fs'; import {executeOneiron} from ${JSON.stringify(resolve("src/factory/adapters/oneiron.ts"))}; import {FactoryStore} from ${JSON.stringify(resolve("src/factory/store.ts"))}; const runtime={now:Date.now,source:async()=>(${JSON.stringify(source)}),status:async()=>{const store=new FactoryStore(${JSON.stringify(database)});try{return {...store.status(),ownerPaused:false}}finally{store.close()}},run:async(argv)=>{writeFileSync(${JSON.stringify(marker)},JSON.stringify({attemptId:process.env.PRIME_FACTORY_ATTEMPT_ID,source:process.env.PRIME_FACTORY_SOURCE_FINGERPRINT,argv}));return ${JSON.stringify(response)}},call:async()=>{throw Error('No model request permitted in fixture')}}; console.log(JSON.stringify(await executeOneiron(${JSON.stringify(retryManifest.path)},${JSON.stringify(permitPin.path)},true,runtime,${JSON.stringify(retryManifest.sha256)})));`,
				);
