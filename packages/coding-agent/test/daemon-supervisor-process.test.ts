...
`,
	...
	it("bounded-stops a wedged root worker while the supervisor and sibling workers survive", {
		tags: ["process-stress"],
		timeout: 60_000,
	}, async () => {
		const root = tempDir();
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `prime-supervisor-wedged-stop-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
