...
describe("AgentSession rlm recursion", () => {
	...
	// Subsumes upstream's host-completion notification assertion and additionally
	// proves the retained terminal child becomes passivation-eligible.
	...
	it.each([
		...
		async ({ memoized, failFirstAppend }) => {
			...
			try {
				...
				expect(attributionAttempts).toBe(failFirstAppend ? 3 : 2);
			} finally {
