...
describe("subagent default model setting", () => {
	...
	it("applies reasoning to the configured subagent model", async () => {
		const harness = await createHarness({
			provider,
			models: [
				{ id: "parent-model", reasoning: false },
		...
		try {
			harness.setResponses([fauxAssistantMessage("child answer")]);
			const result = await harness.session.runRlmChild("do the work", { reasoning: "low" });
			expect(result.model).toBe(`${provider}/child-model`);
			await vi.waitFor(() =>
