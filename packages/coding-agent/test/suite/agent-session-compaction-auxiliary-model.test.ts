...
describe("AgentSession compaction auxiliary model", () => {
	...
	it("does not forward session headers when the auxiliary model has none", async () => {
		completeSimpleMock.mockReset().mockResolvedValue(fauxAssistantMessage("Auxiliary summary"));
		const harness = await createHarness({
			models: [
				{ id: "session-model", name: "Session" },
				{ id: "aux-model", name: "Auxiliary" },
			],
			settings: { auxiliaryModel: "faux/aux-model", compaction: { keepRecentTokens: 1 } },
			persistSession: true,
		});
		try {
					...
					? {
							...
							headers:
								model.id === "session-model"
									? { Authorization: "session-only-test-token", "X-Session-Route": "primary" }
									: undefined,
						}
					: auth;
			});
			...
			expect(calls.length).toBeGreaterThan(0);
			for (const call of calls) {
				expect(call[0]).toMatchObject({ id: "aux-model" });
				const headers = (call[2] as { headers?: Record<string, string> }).headers ?? {};
				expect(headers).not.toHaveProperty("Authorization");
				expect(headers).not.toHaveProperty("X-Session-Route");
			}
		} finally {
			harness.cleanup();
		}
