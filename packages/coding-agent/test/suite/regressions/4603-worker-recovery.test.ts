...
`,
		...
		const listenersAfterShutdown = spawnSync(ssPath, ["-lxp"], {
			encoding: "utf8",
			env: { ...process.env, ...scanEnvironment },
		}).stdout;
		expect(listenersAfterShutdown).not.toContain(`pid=${predecessor.child.pid}`);
		expect(listenersAfterShutdown).not.toContain(`pid=${successor.child.pid}`);
		expect(listenersAfterShutdown).not.toContain(`pid=${workerPid}`);
		expect(listenersAfterShutdown).not.toContain(`pid=${currentSupervisorPid}`);
