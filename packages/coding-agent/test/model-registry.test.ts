...
describe("ModelRegistry", () => {
	...
	describe("live Prime Inference models", () => {
		...
		test.each<[string, string[], Record<string, unknown>]>([
			...
			"restores cached private metadata and reasoning controls for %s only for matching credentials and team",
			async (id, supportedParameters, expectedControls) => {
				...
				).toMatchObject({ name: "Private Deployment", contextWindow: 200_000, ...expectedControls });
				...
				const restoredRegistry = ModelRegistry.create(
				...
				for (const changed of [
					...
					expect(
						(await changedRegistry.refreshAvailableModels()).some((model) => model.id === privateRoute.id),
					).toBe(false);
				}
				...
				const previousFormatRegistry = ModelRegistry.create(authStorage, modelsJsonPath);
				expect(
					(await previousFormatRegistry.refreshAvailableModels()).find((model) => model.id === privateRoute.id),
	...
	describe("API key resolution and stale auth", () => {
		...
		test("preserves stale-auth entitlements only for the same prime-inference team", async () => {
			...
			const headerSpy = vi.spyOn(authStorage, "getProviderHeaders").mockReturnValue({ "X-Prime-Team-ID": "team-a" });
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
