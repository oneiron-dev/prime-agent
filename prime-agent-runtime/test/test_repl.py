...
class SnapshotMemoryAndFileHandleSafetyTest(unittest.TestCase):
    def setUp(self):
        sys.path.insert(0, SRC)
        self.addCleanup(sys.path.remove, SRC)
        import rlm.repl as repl_module
        ...
        self.manifest = os.path.join(self.tmp.name, "kernel-state.json")
    ...
    def test_defaults_bound_aggregate_and_per_variable_state(self):
        self.assertEqual(self.repl.DEFAULT_SNAPSHOT_MAX_BYTES, 64 * 1024 * 1024)
        self.assertEqual(self.repl.DEFAULT_SNAPSHOT_MAX_VARIABLE_BYTES, 8 * 1024 * 1024)

    def test_direct_file_handle_is_purged_after_atomic_snapshot(self):
        target = os.path.join(self.tmp.name, "durable.txt")
        ...
        ns = {"keep": 7, "f": fh}
        result = self.snap(ns)
        self.assertEqual(result["saved"], ["keep"])
        self.assertEqual(result["purgedFileHandles"], ["f"])
        self.assertNotIn("f", ns)
        with open(self.manifest) as manifest_fh:
            manifest = json.load(manifest_fh)
        self.assertEqual(manifest["version"], 2)
        self.assertEqual(manifest["purgedFileHandles"], ["f"])
        with open(target) as durable_fh:
            self.assertEqual(durable_fh.read(), "durable")

    def test_nested_file_handle_reducer_is_never_persisted(self):
        target = os.path.join(self.tmp.name, "nested.txt")
        with open(target, "w") as fh:
            fh.write("durable")
        fh = open(target, "r")
        self.addCleanup(fh.close)
        result = self.snap({"nested": {"f": fh}})
        self.assertEqual(result["saved"], [])
        self.assertEqual(
            result["skipped"],
            [{"name": "nested", "reason": "unsafe dill file-handle reducer"}],
        )

    def test_legacy_write_handle_reducer_is_rejected_before_dill_loads(self):
        import dill

        target = os.path.join(self.tmp.name, "legacy-write.txt")
        fh = open(target, "w")
        fh.write("durable")
        fh.close()
        blob = dill.dumps(fh)
        self.assertTrue(self.repl._has_filehandle_reducer(blob))
        with open(self.path, "wb") as snapshot_fh:
            dill.dump({"f": blob, "keep": dill.dumps(9)}, snapshot_fh)
        ns = {}
    ...
    def test_compaction_keeps_recent_data_and_prunes_all_aggregate_overflow(self):
        ...
        self.assertIn("math", result["saved"])
        self.assertIn("new", result["saved"])
        self.assertNotIn("old", result["saved"])
        self.assertNotIn("middle", result["saved"])
        self.assertEqual(result["pruned"], ["middle", "old"])
