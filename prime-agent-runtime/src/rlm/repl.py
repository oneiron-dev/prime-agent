...
def _snapshot_state(
    ...
    for name in candidate_names:
        ...
        if value is missing:
            ...
            continue
        # dill can serialize even a closed handle and later reopen it through
        # _create_filehandle. Write/append/update modes can truncate durable data.
        ...
        if remaining <= 0:
            ...
            continue
        buffer = _SnapshotBuffer(min(max_variable_bytes, remaining))
        try:
    ...
    try:
        try:
            ...
            pruned = sorted(name for name in set(oversized) if name in ns) if prune_oversized else []
            manifest = {
                "version": 2,
                "savedNames": saved,
                "skipped": skipped,
